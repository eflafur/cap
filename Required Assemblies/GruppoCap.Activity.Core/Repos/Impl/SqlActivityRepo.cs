...
{
    public class SqlActivityRepo : SqlRepositoryBase<Activity>, IActivityRepo
    {
        // GET LATEST
        public ISubCollection<Activity> Filter(Boolean includePrivileged = false, Int32? companyId = null, Int32 howMany = 1000)
        {
            var sql = Sql.Builder.Append(" SELECT TOP {0} * FROM REVO_ACTIVITY ".FormatWith(howMany));
        ...
        private ISubCollection<Activity> FilterByEntity(String filterEntityField, String filterEntityId, String filterEntityType = "", Boolean includePrivileged = false, Int32? companyId = null, Boolean upperizeParameters = false, Int32 howMany = 1000)
        {
            ...
            if (filterEntityId.IsNullOrWhiteSpace() == false)
            {
                ...
                {
                    sql.Append(" ( (UPPER({0}_ENTITY_ID) = @0)", "{1}".FormatWith(filterEntityField.ToUpper(), filterEntityId.ToUpper()));

                    if (filterEntityType.IsNullOrWhiteSpace() == false)
                    {
                        sql.Append(" AND (UPPER({0}_ENTITY_TYPE) = @0)", "{1}".FormatWith(filterEntityField.ToUpper(), filterEntityType.ToUpper()));
                    }
                ...
                {
                    sql.Append(" ( ({0}_ENTITY_ID = @0)", "{1}".FormatWith(filterEntityField.ToUpper(), filterEntityId));

                    if (filterEntityType.IsNullOrWhiteSpace() == false)
                    {
                        sql.Append(" AND ({0}_ENTITY_TYPE = @0)", "{1}".FormatWith(filterEntityField.ToUpper(), filterEntityType));
                    }
