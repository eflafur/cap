...
namespace PetaPoco
{
    ...
    public class Database : IDisposable
    {
        ...
        private void CommonConstruct()
        {
            ...
            if (dbtype.StartsWith("Oracle")) _dbType = DBType.Oracle;
        ...
        {
            _transactionDepth = (_transactionDepth < 0 ? 0 : _transactionDepth) + 1;
        ...
        public void BuildPageQueries<T>(long skip, long take, string sql, ref object[] args, out string sqlCount, out string sqlPage)
        {
            ...
            if (_dbType == DBType.SqlServer || _dbType == DBType.Oracle)
            {
                ...
                // FIX - 30.12.2014 - Alessandro Bertelli - ORDER BY SELECT NULL CRASH ON ORACLE
                if (_dbType == DBType.Oracle)
                {
                    sqlPage = string.Format("SELECT * FROM (SELECT ROW_NUMBER() OVER ({0}) peta_rn, {1}) peta_paged WHERE peta_rn>@{2} AND peta_rn<=@{3}",
                                            sqlOrderBy == null ? "ORDER BY NULL" : sqlOrderBy, sqlSelectRemoved, args.Length, args.Length + 1);
                }
                else
                {
                    sqlPage = string.Format("SELECT * FROM (SELECT ROW_NUMBER() OVER ({0}) peta_rn, {1}) peta_paged WHERE peta_rn>@{2} AND peta_rn<=@{3}",
