...
{
    public class CredentialRepo : RepositoryBase<Credential>, ICredentialRepo
    {
        ...
        public Credential GetCredentialByEmailAndPassword(String email, String password)
        {
            try
            {
                return db.SingleOrDefault<Credential>(" WHERE UPPER(EMAIL) = @0 AND PASSWORD_HASH = @1 ", email.ToUpper(), password);
            }
            catch (Exception ex)
            {
