...
{
    public class SqlUserRepo : SqlRepositoryBase<User>, IUserRepo
    {
        // GET BY ACCOUNT
        public User GetUserByAccount(String userId, String domain)
        {
            try
            {
                var sql = PetaPoco.Sql.Builder.Append(" SELECT * FROM REVO_AUTH_USERS WHERE USER_ID = @0 ", userId.ToUpperInvariant());

                if (domain.IsNullOrWhiteSpace() == false)
                    sql.Append(" AND DOMAIN = @0 ", domain.ToUpperInvariant());
        ...
        {
            var sql = Sql.Builder.Append(" SELECT TOP {0} * FROM REVO_AUTH_USERS ".FormatWith(howMany));
        ...
        public Boolean IsUserEnabledForCompany(Int32 companyId, String userId)
        {
            var sql = Sql.Builder
                ...
                .Append(" WHERE USER_ID = @0 AND COMPANY_ID = @1 ", userId.ToUpper(), companyId)
            ;
        ...
        public IUpdateOperationResult SetUserCompanyAsMain(Int32 companyId, String userId, String applicationId)
        {
            try
            {
                ...
                sql = Sql.Builder.Append(" UPDATE REVO_AUTH_USERS_COMPANIES ")
                                ...
                                .Append(" WHERE COMPANY_ID = @0 AND USER_ID = @1 ", companyId, userId.ToUpper());
