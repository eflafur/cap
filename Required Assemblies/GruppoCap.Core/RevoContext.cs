...
namespace GruppoCap.Core
{
    public class RevoContext : IRevoContext
    {
        ...
        )
        {
            ...
            _Logger = logger.CreateScope("REVO-CTX-{0}".FormatWith(Ambient.Current));

            _cacheProvider = cacheProvider;
        }

        // SERVICE PROVIDER
        public IRevoServiceProvider ServiceProvider
        {
            ...
            get
            {
                if (_Logger == null)
                {
                    try
                    {
                        ...
                        _Logger = GenericTypeResolver.ResolveInstance<ILogger>().CreateScope("REVO-CTX-{0}".FormatWith(Ambient.Current));
                    }
                    catch (Exception)
                    {
                        // USE THE DEFAULT SHARED ONE
                        _Logger = DummyLogger.SharedDefaultLogger.CreateScope("REVO-CTX-{0}".FormatWith(Ambient.Current));
                    }
                }
